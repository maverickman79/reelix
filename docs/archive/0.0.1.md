# Progress archive — 0.0.1

The session-by-session record of how Reelix reached `v0.0.1`, moved here when
0.0.2 began. Newest first, ending with the day the project was bootstrapped.

These entries are kept verbatim. They are the record of how each decision was
reached — what was tried, what the capture showed, what was rejected and why —
and that reasoning outlives the summary of it. The retrospective at the top of
`docs/progress.md` is what these collapse into; anything here that is still
true about the current state is carried there as well.

Twelve entries: Step 7 back through the bootstrap.
